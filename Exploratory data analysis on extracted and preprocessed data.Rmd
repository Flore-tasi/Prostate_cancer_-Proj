...
# Crosscheck
```{r}
unique(surv_data$OS_STATUS)
table(surv_data$OS_STATUS)
```
#Check the actual OS_STATUS values before recoding:
```{r}
table(clinical_data$value[clinical_data$clinicalAttributeId == "OS_STATUS"])
```
...
ggsurvplot(
  ...
  # 🛠️ Clarity improvements for risk table
  risk.table.height = 0.3,               # More vertical space for risk table
  risk.table.y.text.col = TRUE,          # Color text same as curve
  risk.table.y.text = TRUE,              # Show group names
  font.tickslab = 10,                    # Increase tick label font size
  tables.theme = theme_minimal(),        # Match minimal theme
  font.risk.table = c(10, "plain", "black") # Set font size, style, and color
)
```
...
ggsurvplot(
  ...
  conf.int = FALSE,
  xlim = c(0, 300),
  break.time.by = 20,                   # Adjust spacing for readability
  pval = TRUE,
  surv.median.line = "hv",
  palette = "Set1",
  ggtheme = theme_minimal(),

  # 🛠️ Clarity Enhancements
  risk.table.height = 0.3,              # More vertical space for table
  risk.table.y.text.col = TRUE,         # Color labels same as lines
  risk.table.y.text = TRUE,             # Show group names
  font.tickslab = 10,                   # Bigger axis font
...
cna_burden <- cna_data %>%
  group_by(sampleId) %>%
  summarise(CNA_burden = sum(abs(as.numeric(alteration)) > 1, na.rm = TRUE))
```
# Visuslize the distribution
```{r}
ggplot(cna_burden, aes(x = CNA_burden)) +
  geom_histogram(bins = 30, fill = "steelblue") +
  theme_minimal() +
  labs(
    title = "CNA Burden per Sample",
    x = "Number of Altered Genes (|CNA| > 1)",
    y = "Number of Samples"
  )
```
...
cna_burden <- cna_data %>%
  group_by(sampleId) %>%
  summarise(CNABurden = sum(abs(as.numeric(alteration)) > 1, na.rm = TRUE))
```
...
```
#Log-transform the matrix (to reduce outlier impact)
```{r}
mut_mat_log <- log1p(mut_mat[,-1])  # log(1 + x) to preserve 0s
pca_result <- prcomp(mut_mat_log, scale. = TRUE)
```
#Remove extreme hypermutated samples
```{r}
mut_counts <- rowSums(mut_mat[,-1])
threshold <- quantile(mut_counts, 0.99)  # Keep bottom 99%
filtered_mat <- mut_mat[mut_counts < threshold, -1]
```
# Remove columns (genes) with zero variance and run PCA
```{r}
filtered_mat_nzv <- filtered_mat[, apply(filtered_mat, 2, var) != 0]
pca_result <- prcomp(filtered_mat_nzv, scale. = TRUE)
```
# Umap
```{r}
library(uwot)
umap_result <- umap(mut_mat[,-1])
...
# clear console
```{r}
cat("\014")          # Clear console (same as Ctrl+L in RStudio)
```
